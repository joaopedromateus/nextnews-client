...
const TechnologyPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:5000/api/articles?category=Security') // Use "Technology" com a primeira letra maiúscula
      .then((response) => response.json())
      .then((data) => {
        // Filtra apenas as notícias da categoria "Tecnologia"
        const technologyNewsFiltered = data.filter((article: NewsItem) => article.category === 'Security');
